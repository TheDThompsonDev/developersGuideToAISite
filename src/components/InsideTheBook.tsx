...
export function InsideTheBook() {
  return (
    <section
      id="excerpt"
      className="relative py-28 lg:py-36 border-b border-ink-3 overflow-hidden"
    >
      <div className="relative mx-auto max-w-5xl px-6 lg:px-8">
        ...
        <div className="mb-12 lg:mb-16">
          ...
          <p className="mt-6 text-lg text-bone-muted max-w-2xl leading-relaxed">
            Four excerpts, pulled directly from the book. A maxim, two
            production code samples, and a decision framework. This is the voice
        ...
        <div className="excerpt-viewer">
          ...
          <input
            ...
            id="excerpt-tab-1"
            defaultChecked
            className="sr-only peer/t1"
          />
          <input
            type="radio"
            name="excerpt-tab"
            id="excerpt-tab-2"
            className="sr-only peer/t2"
          />
          <input
            type="radio"
            name="excerpt-tab"
            id="excerpt-tab-3"
            className="sr-only peer/t3"
          />
          <input
            type="radio"
            name="excerpt-tab"
            id="excerpt-tab-4"
            className="sr-only peer/t4"
          />

          {/* Tab labels */}
          <div
            className="excerpt-tabs"
            role="tablist"
          ...
          >
            ...
            <label htmlFor="excerpt-tab-2" className="excerpt-tab" data-tab="2">
              ...
              <span className="excerpt-tab-label">Vector Database</span>
          ...
          <div className="excerpt-panels">
                ...
                <div className="mt-10 lg:mt-12 grid lg:grid-cols-12 gap-8">
                  ...
                  <div className="lg:col-span-10">
                    <p className="text-lg text-bone-muted leading-relaxed">
                      Every retrieval trick in this chapter - better chunking,
                      smarter retrieval, re-ranking, citations - depends on
                      having clean, current, and trustworthy sources. Treat data
                      quality like a first-class feature of your pipeline, not
                      an afterthought.
                    </p>
                    <p className="mt-5 font-display-italic text-xl text-bone">
                      "When answers are wrong, observability is how you debug
            ...
            {/* ============ TAB 2: VECTOR DATABASE CODE (Python) ============ */}
            <article className="excerpt-panel" data-panel="2">
              ...
              <div className="px-4 py-8 lg:px-12 lg:py-12">
                <p className="text-bone-muted mb-8 leading-relaxed max-w-2xl">
                  A production vector database requires robust data ingestion. Here we build a
                  document store that handles chunking, deterministic IDs, and
                  metadata preservation, ensuring we can trace every retrieved
                  snippet back to its source.
                </p>
                ...
                <pre className="excerpt-code">
                  <code>
                    ...
                    <span className="code-type">MultiDocumentVectorStore</span>:
                    {"\n"}
                    {"    "}
                    <span className="code-marker">❶</span>{" "}
                    <span className="code-kw">def</span>{" "}
                    <span className="code-fn">__init__</span>(self, documents:
                    List[Document]):{"\n"}
                    {"        "}embedding_function ={" "}
                    <span className="code-fn">OllamaEmbeddingFunction</span>(
                    {"\n"}
                    {"            "}url=
                    ...
                    ,{"\n"}
                    {"            "}model_name=
                    <span className="code-string">"mxbai-embed-large"</span>
                    {"\n"}
                    {"        "}){"\n"}
                    {"        "}client = chromadb.
                    <span className="code-fn">Client</span>(){"\n"}
                    {"        "}self.collection: Collection = client.
                    <span className="code-fn">create_collection</span>({"\n"}
                    {"            "}name=
                    <span className="code-string">"examples_readme"</span>,
                    {"\n"}
                    ...
                    {"        "}
                    <span className="code-marker">❷</span>{" "}
                    <span className="code-kw">for</span> doc_idx, document{" "}
                    <span className="code-kw">in</span>{" "}
                    <span className="code-fn">enumerate</span>(documents):{"\n"}
                    {"            "}splitter = RecursiveCharacterTextSplitter.
                    <span className="code-fn">from_language</span>({"\n"}
                    {"                "}language=
                    <span className="code-string">"markdown"</span>, chunk_size=
                    <span className="code-const">1500</span>){"\n"}
                    {"            "}document_chunks = splitter.
                    <span className="code-fn">split_text</span>
                    (document.content){"\n"}
                    {"\n"}
                    {"            "}
                    <span className="code-kw">for</span> chunk_idx,
                    document_chunk <span className="code-kw">in</span>{" "}
                    <span className="code-fn">enumerate</span>(document_chunks):
                    {"\n"}
                    {"                "}self.collection.
                    <span className="code-fn">add</span>({"\n"}
                    {"                    "}documents=document_chunk,{"\n"}
                    {"                    "}
                    <span className="code-marker">❸</span> ids=
                    <span className="code-string">f"doc_</span>
                    {"{doc_idx + 1}"}
                    <span className="code-string">_</span>
                    {"{chunk_idx + 1}"}
                    <span className="code-string">"</span>,{"\n"}
                    {"                    "}
                    <span className="code-marker">❹</span> metadatas={"{"}
                    <span className="code-string">"source_url"</span>:
                    document.source_url{"}"}
                    {"\n"}
                    {"                "}){"\n"}
                    {"\n"}
                    {"    "}
                    <span className="code-kw">def</span>{" "}
                    <span className="code-fn">query</span>(self, question:{" "}
                    <span className="code-type">str</span>):{"\n"}
                    {"        "}results = self.collection.
                    <span className="code-fn">query</span>
                    (query_texts=[question], n_results=
                    <span className="code-const">3</span>){"\n"}
                    {"        "}document_chunk_results = results.
                    <span className="code-fn">get</span>(
                    <span className="code-string">'documents'</span>)[
                    <span className="code-const">0</span>]{"\n"}
                    {"        "}
                    <span className="code-marker">❺</span>{" "}
                    document_chunk_metadatas = results.
                    <span className="code-fn">get</span>(
                    <span className="code-string">'metadatas'</span>)[
                    <span className="code-const">0</span>]{"\n"}
                    {"        "}documents: List[Document] = []{"\n"}
                    {"\n"}
                    {"        "}
                    <span className="code-marker">❻</span>{" "}
                    <span className="code-kw">for</span> result_idx,
                    document_chunk <span className="code-kw">in</span>{" "}
                    <span className="code-fn">enumerate</span>
                    (document_chunk_results):{"\n"}
                    {"            "}documents.
                    <span className="code-fn">append</span>(Document({"\n"}
                    {"                "}
                    <span className="code-marker">❼</span>{" "}
                    source_url=document_chunk_metadatas[result_idx].
                    <span className="code-fn">get</span>(
                    <span className="code-string">'source_url'</span>),{"\n"}
                ...
                <ul className="mt-8 grid md:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                  <li className="flex gap-3 text-bone-muted">
                    ...
                    <span>
                      Initializes a local Ollama embedding function and ChromaDB
                      collection.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❷</span>
                    <span>
                      Loops through documents and chunks them using a
                  ...
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❸</span>
                    <span>
                      Assigns a deterministic composite ID based on the document
                      and chunk indices.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❹</span>
                    <span>
                      Attaches metadata to track the original source URL for
                      each chunk.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❺</span>
                    <span>
                      Retrieves the metadata array from the vector search
                      results.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                  ...
                  <li className="flex gap-3 text-bone-muted md:col-span-2">
                    <span className="code-marker shrink-0">❼</span>
                    <span>
                      Preserves the original source URL metadata into the
                      retrieved objects.
                    </span>
                  </li>
                </ul>
            ...
            <article className="excerpt-panel" data-panel="3">
              ...
              <div className="px-6 py-10 lg:px-12 lg:py-12">
                <p className="text-bone-muted mb-8 leading-relaxed max-w-2xl">
                  Here's how the book teaches an agent to <em>do</em> things,
                  not just talk about them. The{" "}
                  <code className="text-signal font-mono text-sm">@tool</code>{" "}
                  decorator registers a Python function - and the docstring is
                  what the LLM reads to decide when and how to call it.
                </p>
                ...
                <pre className="excerpt-code">
                  <code>
                    ...
                    {"\n"}
                    <span className="code-kw">
                      from
                    </span> langchain_core.tools{" "}
                    <span className="code-kw">import</span> tool
                    {"\n"}
                    <span className="code-kw">
                      from
                    </span> langgraph.prebuilt{" "}
                    <span className="code-kw">import</span> create_react_agent
                    {"\n\n"}
                    <span className="code-marker">❶</span>{" "}
                    <span className="code-decorator">@tool</span>
                    {"\n"}
                    <span className="code-marker">❷</span>{" "}
                    <span className="code-kw">def</span>{" "}
                    <span className="code-fn">save_file</span>(data:{" "}
                    <span className="code-type">str</span>) -&gt;{" "}
                    ...
                    <span className="code-fn">open</span>(
                    <span className="code-string">"investments.txt"</span>,{" "}
                    ...
                    {"\n"}
                    agent = <span className="code-fn">create_react_agent</span>
                    (model, tools)
                  </code>
                </pre>
                ...
                <ul className="mt-8 grid md:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❶</span>
                    <span>
                      The <code className="text-signal font-mono">@tool</code>{" "}
                      decorator registers the function with the agent runtime.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❷</span>
                    <span>
                      Type hints matter - the agent uses them to validate
                      inputs.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❸</span>
                    <span>
                      The docstring <em>is</em> the instruction manual. The LLM
                      reads it.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted">
                    <span className="code-marker shrink-0">❹</span>
                    <span>
                      A real filesystem operation - the agent can now persist
                      state.
                    </span>
                  </li>
                  <li className="flex gap-3 text-bone-muted md:col-span-2">
                    <span className="code-marker shrink-0">❺</span>
                    <span>
                      The tool is passed to the agent. It can now decide when to
                      call it.
                    </span>
                  </li>
                </ul>
            ...
            <article className="excerpt-panel" data-panel="4">
              ...
              <div className="px-4 py-8 lg:px-12 lg:py-12">
                <p className="text-bone-muted mb-8 leading-relaxed max-w-2xl">
                  Every AI codebase has to pick an abstraction level - raw SDK,
                  lightweight wrapper, or full framework. The book's trade-off
                  curve:
                </p>
                ...
                <blockquote className="mt-10 pt-8 border-t border-ink-3">
                  <p className="font-display-italic text-2xl lg:text-3xl text-bone leading-snug">
                    "Choose the level that removes your biggest current
                    bottleneck - and be ready to slide up or down the stack as
                    those bottlenecks change."
                  </p>
                </blockquote>
              </div>
            </article>
